...
procedure Safe_Read (Fd : int; Buf : out Init_String; Has_Read : out ssize_t) with
  SPARK_Mode
is
...
begin
   loop
      ...
      exit when (Has_Read < 0 and then Errors.Get_Errno /= Errors.ADA_EINTR) or else Has_Read >= 0;
   end loop;
end Safe_Read;
